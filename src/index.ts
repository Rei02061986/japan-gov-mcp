...
server.tool('estat_browse_indicators',
  ...
  {
    tableId: z.string().optional().describe('SSDSテーブルID（例: 0000020101=人口, 0000020103=経済, 0000020104=行政）'),
    keyword: z.string().optional().describe('検索キーワード（例: 事業所, 人口, 財政）'),
    section: z.string().optional().describe('セクションコード（A=人口, C=経済, D=行政）'),
...
server.tool('estat_check_availability',
  ...
  {
    metricIds: z.array(z.string()).describe('チェックする指標IDの配列（例: ["population","revpar","fiscal_strength_index"]）'),
...
server.tool('estat_merger_check',
  ...
  {
    codes: z.array(z.string()).describe('市区町村コードの配列（5桁、例: ["35211","14382"]）'),
    years: z.array(z.number()).optional().describe('チェック対象年のリスト（例: [2000,2005,2010,2015,2020]）'),
...
server.tool('estat_compare_municipalities',
  ...
  {
    codes: z.array(z.string()).describe('市区町村コード配列（5桁、例: ["35211","14382","10426"]）'),
    indicators: z.array(z.string()).describe('SSDS指標コード配列（例: ["A1101","C2108","C210847","D2201"]）'),
    year: z.string().optional().describe('取得年（例: "2020"）。省略時は最新データ'),
    tableId: z.string().optional().describe('SSDSテーブルID（例: "0000020101"）。省略時は指標コードから自動判定'),
  },
  async (p) => {
    const e = need('ESTAT_APP_ID', C.estat.appId); if (e) return txt(e);
    ...
    for (const code of p.codes) {
      ...
      for (const tbl of tableIds) {
        try {
          const res = await estat.getStatsData(C.estat, {
            statsDataId: tbl,
            cdArea: code,
            cdTime: p.year,
            limit: 10000,
            ...
            if (Array.isArray(values)) {
              for (const v of values) {
                ...
                if (cat && p.indicators.includes(cat)) {
                  const val = v['$'];
                  allData[code][cat] = val === '-' || val === 'x' || val === '***' ? null : parseFloat(val);
                }
              }
            }
          } else {
...
server.tool('estat_time_series',
  ...
  {
    code: z.string().describe('市区町村コード（5桁）'),
    indicators: z.array(z.string()).describe('SSDS指標コード配列（例: ["A1101","C2108","D2201"]）'),
    yearFrom: z.number().optional().describe('開始年（デフォルト: 2005）'),
    yearTo: z.number().optional().describe('終了年（デフォルト: 2023）'),
  },
  async (p) => {
    const e = need('ESTAT_APP_ID', C.estat.appId); if (e) return txt(e);
            ...
            for (const v of values) {
              ...
              if (cat === ind) {
                ...
                if (year >= yearFrom && year <= yearTo) {
                  const val = v['$'];
                  yearData[ind][year] = val === '-' || val === 'x' || val === '***' ? null : parseFloat(val);
                }
              }
            }
          }
...
server.tool('estat_correlation',
  '【自治体分析】複数自治体データの指標間相関分析。ピアソン相関係数・解釈（強い/中程度/弱い/無相関）を算出。恣意性排除のため全ペアを表示',
  {
    data: z.array(z.object({
      code: z.string().describe('市区町村コード'),
      name: z.string().describe('市区町村名'),
    }).catchall(z.union([z.string(), z.number(), z.null()]))).describe('自治体データ配列（各行に指標値を含む）'),
    metricKeys: z.array(z.string()).describe('相関を計算する指標キーの配列（例: ["aging_rate","fiscal_strength_index"]）'),
  },
  async (p) => {
...
server.tool('estat_session_init',
  ...
  {
    codes: z.array(z.string()).describe('分析対象の市区町村コード配列（5桁）'),
    years: z.array(z.number()).optional().describe('分析対象年リスト（例: [2000,2005,2010,2015,2020]）'),
    requestedMetrics: z.array(z.string()).optional().describe('利用したい指標IDリスト（例: ["population","revpar","establishments_accom"]）'),
