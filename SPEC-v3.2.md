# japan-gov-mcp v3.2 — 仕様書

> 日本の中央省庁API統合MCPサーバー。36 APIを12ツール・73アクションで統合アクセス。
...
| 対応API数 | 36 |
| 内部データファイル | 5 |
| テスト数 | 569 (564 pass / 0 fail / 5 skip) |
| E2Eシナリオ | 10本 |
| ランタイム | Node.js >= 22 |
...
│  estat / stats / corporate / weather /  │
│  law / geo / academic / opendata / misc │
└─────────────────────────────────────────┘

上位ツール（10-12）は下位ツール（1-9）のhandlerを内部直接呼出し。
...
| frequency | `"year" \| "month" \| "quarter"` | No | fill_gaps | 時系列頻度 |
...
```typescript
{
  ...
  data?: T;
  error?: string;
  source: string;      // "e-Stat/search", "resolve/area_bridge" 等
...
stats(dash/boj/ndb), weather(全12), law(全6), geo(全6), academic(全10), opendata(gov/geo), misc(safety/kkj/mirasapo/mirasapo_cat/mirasapo_region), resolve(area_bridge/time_bridge/code_lookup), navigate(recommend/coverage), join(normalize/fill_gaps)
